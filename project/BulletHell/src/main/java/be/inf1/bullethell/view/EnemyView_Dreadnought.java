...
//        this.getChildren().add(r);
        ...
        Image image = ImageController.getImage(LoadedImage.Type.ENEMY_DREADNOUGHT);
        ImageView iv = new ImageView( image );
        iv.setRotate(-90); // the enemy images are facing DOWN by default, we want them facing LEFT 
        iv.setFitWidth( 172 ); // we don't use EnemyView_Dreadnought.WIDTH because we want the hitbox to be a bit smaller than the actual image
        iv.setPreserveRatio(true);
        iv.setSmooth(true);
        
        iv.setX( -40 ); // move a bit to the left visually so the hitbox matches the body of the dreadnought better
    ...
    public Vector2 getCenterPoint() {
        ...
        return new Vector2( this.model.getPosition().x + EnemyView_Dreadnought.WIDTH / 2 - 70, this.model.getPosition().y + EnemyView_Dreadnought.HEIGHT / 2 - 5 );
    }
